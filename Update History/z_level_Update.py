...
class Level():
    def __init__(self, screen, player, room_amount = 10, worldgen = True):
        ...
        if worldgen:
            self.tile_list, self.enemy_list = Worldgen(player, room_amount, (600, 600), self.level).neon_genesis()
...
class Boss_1(Level):
    def __init__(self, screen, player):
        ...
        self.player = player
        self.tile_list = [
            Room([player.spawnpoint[0] - self.base_room_hwidth[0] // 2, player.spawnpoint[1] - self.base_room_hwidth[1] // 2], self.base_room_hwidth, variation = "boss")
        ]
